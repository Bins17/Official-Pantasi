...
function updateCountdown() {
            const targetDate = new Date('2024-01-06T22:00:00'); // Jan 7 9:30 PM
...
// Function to cancel the counter (called from the "Yes" button in the modal)
function cancelCounter() {
  // Retrieve values from the input containers
  var leftTextarea = document.getElementById('leftTextarea').value;
  var rightTextarea = document.getElementById('rightTextarea').value;
...
document.getElementById('cancelCounterButton').addEventListener('click', openModal);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const playerNamesTextbox = document.getElementById("playerNamesTextbox");

  compareButton.addEventListener("click", function () {
    const leftContent = leftTextarea.value.trim();
    const rightContent = rightTextarea.value.trim();

    const leftPlayers = leftContent.split("\n");
    const rightPlayers = rightContent.split("\n");

    const leftPlayerCount = leftPlayers.length;
    const rightPlayerCount = rightPlayers.length;

    let samePlayerCount = 0;
    let cancelCount = 0;
    let samePlayerNames = [];

    for (let i = 0; i < Math.min(leftPlayerCount, rightPlayerCount); i++) {
      const leftName = leftPlayers[i].toLowerCase();
      const rightName = rightPlayers[i].toLowerCase();

      // Count only the players with specific positions
      if (/pg:|sg:|sf:|pf:|c:|\(6\):/.test(leftName) && /pg:|sg:|sf:|pf:|c:|\(6\):/.test(rightName)) {
        if (leftName === rightName) {
          samePlayerCount++;
          samePlayerNames.push(leftPlayers[i]); // Add the original name to the list
        }
        if (leftName.includes("cancel") || rightName.includes("cancel")) {
          cancelCount++;
        }
      }
    }

    playerNamesTextbox.value = samePlayerNames.join("\n");

   
    showAlert(samePlayerCount);
  });
  ...
  function showAlert(samePlayerCount) {
    showNotification(`Cancel: ${samePlayerCount}/9`);
  }
...
function clearContainers() {
  ...
  document.getElementById('playerNamesTextbox').value = "";
